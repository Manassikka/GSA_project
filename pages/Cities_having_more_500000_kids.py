...
Cities_with_more_than_50000_kids = cities[(cities["0-6_population_total"] > 50000)]
with st.expander("View Raw data"):
    st.dataframe(Cities_with_more_than_50000_kids)
m_7 = folium.Map(location=[	28.667856,77.449791], tiles='OpenStreetMap', zoom_start=5)
mc7 = MarkerCluster().add_to(m_7)

# Add points to the map
for idx, row in Cities_with_more_than_50000_kids.iterrows():
    Marker([row['lng'], row['lat']], popup=row['name_of_city']).add_to(mc7)

# Display the map
st.subheader(" Locating the cities having more than 50,000 total kids ")
stf.folium_static(m_7)